...
## v2.2.1 (2026-06-02) - 长期稳定性修复与测试体系完善

### ? 重大改进

修复了 `test_long_term_stability` 用例中的两个关键 Bug，完善了测试体系，新增了 Code Region 和缓存机制的设计文档。
...
- `find_root_binary()` 依赖"已提交页连续"假设，GC 后该假设被打破
- 导致 Phase 3 断电恢复后数据无法恢复（"no data recovered"）

### ? 修正内容

#### 1. `allocate_physical_page()` 空间耗尽检查
- 添加 `gc_head_page == gc_tail_page && valid_page_count > 0` 检查
- 空间耗尽时返回 -1 而非无效 PPN
- 添加详细调试日志打印分配失败时的系统状态

#### 2. 根页恢复改用全量扫描
- `eflash_ftl_init()` 中直接使用 `find_root_full_scan()` 替代 `find_root_binary()`
- 通过最高 `epoch + global_count` 定位根页，GC 后场景兼容

#### 3. 测试体系完善
- 将 `test_long_term_stability` 提取为独立测试文件 `eflash_ftl_tests_stability.c`
- 还原 extension 测试中被屏蔽的 28 个用例
- Makefile 新增 `test-extension` 和 `test-stability` 构建目标

#### 4. 设计文档更新
- `DESIGN_OVERVIEW.md` 升级为 v2.0，新增缓存机制、Code Region 搬移原理、防掉电详解、使用指南
...
- **Code Region 代码区管理**：支持代码从逻辑页搬移到专用物理区域
- **Write-Back 缓存机制**：4 槽位内容缓存，脏页自动刷写
- **`is_page_still_valid()` 函数**：通过 Radix Tree 当前映射判断页有效性
...
| 指标 | 修改前 | 修改后 | 改进 |
|------|--------|--------|------|
| **数据安全性** | ?? 有风险 | ? 绝对安全 | 消除数据损坏风险 |
| **分配效率** | N/A | ??? 高效 | 单次遍历，无额外操作 |
| **扩展可靠性** | ?? 依赖对齐 | ? 双保险 | 任何情况都安全 |
| **代码复杂度** | 简单 | 中等 | 可维护性良好 |
