...
## v2.2.4 (2026-06-03) - 测试报告与覆盖率文档更新

### 📊 文档与测试

- 新增 `TEST_SUMMARY.md`、`TEST_REPORT.md` 测试报告摘要
- 更新 `TEST_COVERAGE_COMPREHENSIVE_REPORT.md`，反映 72 个用例 100% 通过
- 更新 `DESIGN_OVERVIEW.md` 测试体系章节

---

## v2.2.3 (2026-06-03) - 设计文档与仓库说明完善

### 📝 文档

- 大幅更新 `DESIGN_OVERVIEW.md`（v2.0）：缓存机制、Code Region、掉电恢复、测试体系
- 更新 `REPO_WIKI.md`、`TEST_COVERAGE_COMPREHENSIVE_REPORT.md`
- 新增 `BUGFIX_REPORT.md` 长期稳定性修复详细报告

---

## v2.2.2 (2026-06-03) - 构建目标与概要设计补充

### 🔧 改进

- `Makefile` 完善 `test-extension`、`test-stability` 等测试目标
- `DESIGN_OVERVIEW.md` 补充架构与 Code Region 设计说明

---

## v2.2.1 (2026-06-02) - 长期稳定性修复与测试体系重组

### 🔧 重大改进

修复了 `test_long_term_stability` 用例中暴露的两个关键 Bug，完成了测试体系重组，并完善 Code Region 与缓存机制相关文档。

### 📝 问题描述

**Bug A: 物理页分配耗尽时未返回错误**
- `allocate_physical_page()` 无条件移动 `gc_head_page` 指针，不检查目标页有效性
- Head 指针回绕后覆盖有效数据，导致数据静默丢失
- 调用方 `eflash_ftl_write()` 不检查返回值

**Bug B: 断电恢复时 Radix Tree 根页定位错误**
- `find_root_binary()` 依赖"已提交页连续"的假设，GC 后假设被破坏
- 导致 Phase 3 断电恢复后数据无法恢复（"no data recovered"）

### ✅ 修正内容

#### 1. `allocate_physical_page()` 空间耗尽保护
- 添加 `gc_head_page == gc_tail_page && valid_page_count > 0` 检查
- 空间耗尽时返回 -1 而非无效 PPN
- 添加详细调试日志，打印失败时的系统状态

#### 2. 根页恢复改用全量扫描
- `eflash_ftl_init()` 中直接使用 `find_root_full_scan()` 替代 `find_root_binary()`
- 通过最高 `epoch + global_count` 定位根页，GC 后场景仍正确

#### 3. 测试体系重组
- 将 `test_long_term_stability` 提取为独立测试文件 `eflash_ftl_tests_stability.c`
- 在原 extension 测试列表中恢复被注释的 28 个用例
- `Makefile` 新增 `test-extension` 与 `test-stability` 构建目标

#### 4. 配套文档更新
- `DESIGN_OVERVIEW.md` 升级为 v2.0，补充缓存设计、Code Region 搬移原理、掉电恢复、使用指南
- 新增 `BUGFIX_REPORT.md` 详细修复报告

**测试结果：** `make test-stability` 四阶段全部通过（100K 读写 + 10 次掉电 + 100 扇区验证）

---

## v2.2.0 (2026-05-XX) - Code Region 与缓存机制

### ✨ 新特性

- **Code Region 代码区搬移**：支持将冷数据逻辑页迁移到专用物理区域
- **Write-Back 缓存机制**：4 槽位数据缓存，脏页自动刷写
- **`is_page_still_valid()` 辅助函数**：通过 Radix Tree 当前映射判断页有效性
